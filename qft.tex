...
\begin{enumerate}
    \item Explicitly calculating the propagator between $\mathbf x$ and $\mathbf x_0$:
    \begin{align*}
        U&=\bra{\mathbf x}\exp\left(-it\sqrt{\mathbf p^2+m^2}\right)\ket{\mathbf x_0}=\frac 1{(2\pi^2\abs{\mathbf x-x_0)}}\int^\infty_0\dd p\cdot p\sin(p\abs{\mathbf x-\mathbf x_0})e^{-it\sqrt{p^2+m^2}}.
    \end{align*}
    For rational function of $x$ and $t$, $U(t)\sim e^{-m\sqrt{x^2-t^2}}$\footnote{\url{https://physics.stackexchange.com/a/105049}}.
    \item Noting that all Hermitian operators are observable; in relativity, this cannot be true because 2 spatially separated observables cannot affect each other. In other words, we want $\comm{O_1}{O_2}=0$ for spatially separated $O_1$, $O_2$.
\end{enumerate}
...
\begin{equation}
    \bra\Omega T\phi_H(x_1)\phi_H(x_2)\ket\Omega=\lim_{T\to\infty(1-i\epsilon)}\frac{\int\mathcal D\phi\phi(x_1)\phi(x_2)\exp\left[i\int^T_{-T}\dd^4x\mathcal L\right]}{\int\mathcal D\phi\exp\left[i\int^T_{-T}\dd^4x\mathcal L\right]}
\end{equation}
